          ...
          <view :style="{ top: positionTop + 'rpx' }" class="days">
            <view class="item" v-for="(item, index) in dates" :key="index">
              <view
                ...
                :class="{
                  choose:
                    getActday(`${item.year}-${item.month}-${item.date}`) &&
                    item.isCurM, //选中的日期
                  chooseMarkDay: getChoose(
              ...
              <!-- 增加自定义打卡标记点 2024-04-08 -->
              <view
                v-if="item.isCurM"
                :class="{
                  workDay: isSigned(item),
                  notSigned: isNotSigned(item)
                }"></view>
              <view
                class="iconfont iconyixuanzhong"
                :class="{
                  markDays: getChoose(`${item.year}-${item.month}-${item.date}`)
                }"
                v-if="
...
export default {
  ...
  computed: {
    ...
    height() {
      return (this.dates.length / 7) * 81 + 'rpx';
    }
  },
  methods: {
    // 请求节假日信息
    ...
    isNotSigned(item) {
      return (
        this.showNotSign &&
        this.isWorkDay(item.year, item.month, item.date) &&
        this.isBeforeToday(item) &&
        !this.isSigned(item)
      );
    },
    // 是否是今天之前的日期
    isBeforeToday({ year, month, date }) {
      const day = new Date(`${year}-${month}-${date}`).getTime();
      const { date: today } = this.getToday();
      return day < new Date(today).getTime();
    },
    //已被投标的日期
    getChoose(day) {
